...
Los dos primeros supuestos se pueden evaluar simplemente con un diagrama de dispersión, mientras que para los últimos basta con mirar los datos y evaluar el diseño que tenemos.''')
    st.image('https://estamatica.net/wp-content/uploads/2021/06/correlacion-valores.jpg')
    st.write('''
...
Comidas que mas le gusta al encuestado.\n

**Ejemplo**: Ceviche, arroz con pato, etc.
...
Matrices: Alex Narvaez https://www.youtube.com/watch?v=OyNXw80YgXc&t=412s

Streamlit: Canal:Pycon Chile https://www.youtube.com/watch?v=cZSzK2uIqjY&t=1017s

Streamlit: Andy McDonald https://www.youtube.com/watch?v=3f-j-PZ5N8A&t=374s&pp=ugMICgJlcxABGAE%3D''')
...
options= st.sidebar.selectbox('Paginas',options=['Presentación','Base teórica', 'Propuesta', 'Validación de resultados','Conclusiones y referencias'])
if options=='Presentación':
    ...
    propuesta()
elif options=='Validación de resultados':
    st.write('''### Validación de resultados''')
    st.write('''\nA continuación se describirá el proceso de validación de resultados:\nDe un grupo de encuestados se encontrará el índice de similitud en gustos de comidas usando el coeficiente de correlación de Pearson. Primero se ordenarán los coeficientes de correlación de Pearson en una matriz de tal manera que se pueda ver el índice de similutud entre cualquiera de los encuestados, de forma similar a una tabla de doble entrada, primero se utilizará la librería "Pandas" para encontrar los coeficientes de correlación, posteriormente se utilizará un algoritmo propio. Para una mejor visualización de datos también se mostrará una gráfica de calor y también se realizará la busqueda de los encuestados con el índice de correlación mas alta, lo que podemos interpretar en primera instancia como las personas mas similares en gustos de comidas.\n''')
    st.write('''##### Matríz de correlación(Pandas)''')
    st.write(m_corr_PANDAS)
    st.write('''##### Gráfica de calor(Pandas)''')
    grafcalor()   
    st.write('''##### Búsqueda de los coeficientes de correlación mas altos(Pandas)''')
    ...
    st.write('''##### Matríz de correlación(Algoritmo propio)''')
    daf
    st.write('''##### Gráfica de calor(Algoritmo propio)''')
    grafcalor2()   
    st.write('''##### Búsqueda de los coeficientes de correlación mas altos(Algoritmo propio)''')
    st.write('El primer coeficiente de correlación mas alto es:\n ')
